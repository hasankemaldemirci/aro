...
export function analyzeMetrics(
  ...
  CONTEXT_FILES.forEach((file) => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (content.length > 500) fileScore += 15;
      if (content.length > 2000) fileScore += 10; // Depth bonus for comprehensive docs
      if (
      ...
      // Penalize generic ARO-generated templates — reward project-specific customization
      const isGenericTemplate =
        content.includes("ARO: AI Agent Constitutional Rules") ||
        content.includes("Optimized by ARO") ||
        content.includes("Generated by ARO (Agent Readability Optimizer)");
      if (isGenericTemplate) fileScore -= 25;

      metrics.contextFiles.push({
        name: file,
        size: content.length,
        score: Math.min(Math.max(fileScore, 0), 100),
      });

      if (fileScore < 80) {
        const reason = isGenericTemplate
          ? `Generic ARO template detected in ${file} - Customize with project-specific instructions to improve score.`
          : `Low quality instructions in ${file} - Add more 'Always'/'Avoid' constraints to reduce agent confusion.`;
        metrics.blindSpots.push(reason);
      }
    }
  });
