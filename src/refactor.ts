...
export async function run() {
  ...
  if (!fs.existsSync(contextPath)) {
    console.log(
      Branding.error(
        "❌ Error: No .agent_context_pro.json found. Run analysis first.",
      ),
    );
    process.exit(1);
  }
  ...
  console.log(
    Branding.magenta.bold(
      "\n🛠️  ARO Auto-Refactor: Commencing Optimization...",
    ),
  );

  // 1. Documentation Optimization
  if (!metrics.hasReadme) {
    console.log(Branding.warning("📝 Generating missing README.md..."));
    ...
    console.log(Branding.success("✅ README.md created."));
  }
  ...
  for (const [file, content] of Object.entries(requiredConfigs)) {
    if (!fs.existsSync(path.join(projectPath, file))) {
      console.log(
        Branding.warning(`🛠️  Recovering missing config: ${file}...`),
      );
      fs.writeFileSync(path.join(projectPath, file), content);
      console.log(Branding.success(`✅ ${file} generated.`));
    }
  }
...
function optimizeFile(filePath: string) {
  try {
    ...
    if (lines.length > 300) {
      ...
      console.log(
        Branding.warning(
          `⚠️  Self-Healing: Detected high-complexity in ${path.basename(filePath)} (${lines.length} lines). Applying advisory markers.`,
        ),
      );
    } else {
      console.log(
        Branding.gray(
          `📌 Adding AI-SEO markers to: ${path.basename(filePath)}`,
        ),
      );
    }
