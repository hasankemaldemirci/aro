...
export async function run(options?: { apply?: boolean }) {
  ...
  if (!fs.existsSync(contextPath)) {
    console.log(
      Branding.error(
        "❌ Error: No .agent_context_pro.json found. Run 'aro audit' first.",
      ),
    );
    process.exit(1);
  }
  ...
  console.log(
    Branding.magenta.bold(
      shouldApply
        ? "\n🤖 ARO Agentic Refactor: Analyzing and applying optimizations..."
        : "\n🔍 ARO Refactor Analysis: Scanning for optimization opportunities...",
    ),
  );
  ...
  if (refactorSuggestions.length > 0) {
    console.log(
      Branding.warning(
        `\n🔍 Found ${largeFilesFound} file(s) exceeding 300 lines:\n`,
      ),
    );
    ...
    refactorSuggestions.forEach(({ file, lines, suggestions, content }) => {
      console.log(Branding.error(`📄 ${file} (${lines} lines)`));

      // Add to markdown
      markdownReport += `## 📄 \`${file}\` (${lines} lines)\n\n`;
      markdownReport += `### Recommended Actions:\n\n`;

      suggestions.forEach((suggestion) => {
        console.log(Branding.gray(`   → ${suggestion}`));
      ...
      if (shouldApply && content) {
        ...
        if (appliedChanges.length > 0) {
          console.log(Branding.success(`   ✅ Applied refactoring:`));
          appliedChanges.forEach((change) => {
            console.log(Branding.gray(`      • ${change}`));
          });
          markdownReport += `\n**Applied Changes:**\n`;
          appliedChanges.forEach((change) => {
            markdownReport += `- ✅ ${change}\n`;
          });
          markdownReport += `\n`;
        }
      }
    ...
    console.log(
      Branding.success(`\n📝 Refactoring plan saved to: ARO-REFACTOR-PLAN.md`),
    );

    if (shouldApply) {
      console.log(
        Branding.success(
          "\n✨ Refactoring applied! Review the changes and run 'aro audit' to see updated scores.",
        ),
      );
    }
  } else {
    console.log(
      Branding.success(
        "\n✅ No files exceeding 300 lines found. Your codebase is well-structured!",
      ),
    );
  }
  ...
  if (!shouldApply) {
    console.log(
      Branding.cyan(
        "\n💡 Tip: Run 'aro refactor --apply' to automatically apply these suggestions.",
      ),
    );
  }
}
